<template>
	<view class="bigbox">
		...
		<view class="box" v-for="(item,index) in listItems" @click="junp(item)">
			<view class="imgbox">
				<p class="tips" v-if="item.codeState==3">已揭晓</p>
				<p class="tips_jx" v-if="item.codeState==1">进行中</p>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			var a = this.index + '/10/1/' + num
			getUserBuyList(i, a).then(res => {
				this.listItems = res.data.listItems
			})
		},
		methods: {
			junp(e){
				console.log(e.codeState);
				if(e.codeState==1){
					uni.navigateTo({
						url:'/pages/index/product?id='+e.od+'&sid='+e.sid
					})
				}else{
					uni.navigateTo({
						url:'/pages/index/revealing?id='+e.od+'&sid='+e.sid
					})
				}
			},
			sjc(value) {
